...
MUTATION CONTROL. T2_MUTATE=1 spells Poisson's ratio <v> in the "wrong"
slot too — the pathology removed. Nothing is rejected and
'nu_instead_of_v=reproduced' is no longer printed.
"""
...
def main() -> int:
    if MUTATE:
        print("mutation=the_wrong_slot_spells_poissons_ratio_v")
    return L.parse_error(
        "nu_instead_of_v",
        wrong=L.solid_deck() if MUTATE
        else L.solid_deck(material=MAT_NU),
        right=L.solid_deck(),
        message='tag "nu"',
