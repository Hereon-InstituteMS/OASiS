...
# Tier-2 for fourc::thermo_transient_mms#3 — a plain 'DESIGN LINE DIRICH
# CONDITIONS' block with VAL [1.0] and FUNCT [<id>] applies VAL * f(x,t),
# re-evaluated at the node coordinates and the CURRENT time every step.  No
# THERMO-prefixed section is needed.
#
# Two boundary nodes are result-tested at the final time against the analytic
# u*(x,t) to 1e-12 — a tolerance only an exactly-tracked boundary can meet:
#
#   node  5 at (0.5, 0.0) : u*(t=0.4) = 0.7977457514062631
#   node 27 at (1.0, 0.25): u*(t=0.4) = 0.5954915028125263
#
# The FUNCT [0] arm also FALSIFIES a detail of the entry's Signal, which said
# the frozen boundary equals "the t=0 trace".  It does not: with no function
# the applied value is VAL alone, so both nodes sit at exactly 1.0, while
# their t=0 traces are 1.25 and 1.5.  A reader checking for the t=0 trace
# would conclude the BC was fine.
...
mms "{\"n\":8,\"dt\":0.05,\"numstep\":8,\"results\":[[5,0.7977457514062631,1e-12],[27,0.5954915028125263,1e-12]]}" \
    "$TMP/with_funct.yaml"
mms "{\"n\":8,\"dt\":0.05,\"numstep\":8,\"dirich_funct\":0,\"results\":[[5,0.7977457514062631,1e-12],[27,0.5954915028125263,1e-12]]}" \
    "$TMP/no_funct.yaml"
# recording run for the frozen arm, to read the actual boundary values
mms "{\"n\":8,\"dt\":0.05,\"numstep\":8,\"dirich_funct\":0,\"results\":[[5,0.0,1e-30],[27,0.0,1e-30]]}" \
    "$TMP/frozen_record.yaml"

probe WITH_FUNCT    "$TMP/with_funct.yaml"
probe NO_FUNCT      "$TMP/no_funct.yaml"
probe FROZEN_RECORD "$TMP/frozen_record.yaml"

echo "TRACKING_TESTS_CORRECT=$(grep -c 'is CORRECT' "$TMP/WITH_FUNCT.log")"
grep -m1 -F "processor 0 finished normally" "$TMP/WITH_FUNCT.log"
grep -m1 -F "is WRONG --> actresult=" "$TMP/NO_FUNCT.log"

# What the frozen boundary actually holds: VAL, not the t=0 trace.
echo "FROZEN_AT_VAL=$(grep -c 'is WRONG --> actresult= 1.00000000000000000e+00' "$TMP/FROZEN_RECORD.log")"
echo "FROZEN_AT_T0_TRACE=$(grep -cE 'actresult= 1\.2500000|actresult= 1\.5000000' "$TMP/FROZEN_RECORD.log")"
exit 0
