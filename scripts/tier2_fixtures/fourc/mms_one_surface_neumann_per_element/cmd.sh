...
# Tier-2 for fourc::thermo_transient_mms#1 — in a 2D Thermo discretisation
# the PLAIN 'DESIGN SURF NEUMANN CONDITIONS' on a DSURF covering every node
# IS the body load: the condition geometry is the QUAD4 elements themselves
# and TemperImpl::radiation() integrates ONOFF*VAL*FUNCT(x,t) over each one.
#
# The constraint that follows from that is the one worth pinning: at most ONE
# such condition may touch an element.  Add a second DSURF over the same
# nodes and 4C throws from src/thermo/src/element/4C_thermo_ele_impl.cpp —
# not at input read, but on the first element evaluation.
#
#   ONE       one condition  -> body load applied, 'is CORRECT', exit 0
#   OVERLAP   two conditions -> 'more than one VolumeNeumann cond on one node'
#   NONE      no condition   -> parses and runs, source silently absent
#
# The NONE arm is the reason the throw matters: 4C has no complaint for a
# MISSING body load, only for a duplicated one.
...
CENTRE=145            # node at (0.5, 0.5) of the 16x16 mesh
EXACT=-0.011271242968684092   # u*(0.5,0.5,0.4), analytic

mms "{\"n\":16,\"dt\":0.05,\"numstep\":8,\"results\":[[$CENTRE,$EXACT,5e-3]]}" \
    "$TMP/one.yaml"
mms "{\"n\":16,\"dt\":0.05,\"numstep\":8,\"neumann_twice\":true,\"results\":[[$CENTRE,$EXACT,5e-3]]}" \
    "$TMP/overlap.yaml"
mms "{\"n\":16,\"dt\":0.05,\"numstep\":8,\"neumann\":\"\",\"results\":[[$CENTRE,$EXACT,5e-3]]}" \
    "$TMP/none.yaml"
...
grep -m1 -F "is CORRECT" "$TMP/ONE.log"
grep -m1 -F "processor 0 finished normally" "$TMP/ONE.log"
grep -m1 -F "more than one VolumeNeumann cond on one node" "$TMP/OVERLAP.log"
grep -m1 -F "4C_thermo_ele_impl.cpp" "$TMP/OVERLAP.log"

# The duplicate is caught during element evaluation, not at input read: the
# discretisation is fully built first.
if [ "$(grep -c 'fill_complete() on discretization thermo' "$TMP/OVERLAP.log")" -gt 0 ]; then
  echo "OVERLAP_BUILT_DISCRETISATION=yes"
else
  echo "OVERLAP_BUILT_DISCRETISATION=no"
fi
# An ABSENT body load, by contrast, draws no comment at all.
grep -m1 -F "is WRONG --> actresult=" "$TMP/NONE.log"
echo "MISSING_BODY_LOAD_WARNINGS=$(grep -ciE 'neumann.*(missing|absent|no condition)|no body load' "$TMP/NONE.log")"
exit 0
