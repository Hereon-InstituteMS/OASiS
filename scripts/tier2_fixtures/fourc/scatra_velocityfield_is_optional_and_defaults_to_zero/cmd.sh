...
# Tier-2 for fourc::scalar_transport#1, and the execution that CORRECTED it.
#
# The entry used to say the omission of VELOCITYFIELD raised "requested
# velocity field not found" or "Vector velnp uninitialized".  Neither string
# exists.  'zero' is the DEFAULT, so a deck that simply leaves the key out is
# not merely accepted, it produces the identical answer:
#
#   EXPLICIT  VELOCITYFIELD: "zero"    -> result test passes, exit 0
#   OMITTED   key deleted entirely     -> result test passes, exit 0, and the
#                                         word "velocit" appears zero times
#                                         anywhere in the log
#   BOGUS     VELOCITYFIELD: "function" with no VELFUNCNO -> 4C DOES object
#
# The third arm matters: it shows 4C is willing to complain about this key
# when there is something to complain about, so the silence in the second arm
# is a measurement rather than an absence of checking.
...
NODE=13     # centre of the 4x4 mesh, at (0.5, 0.5)
TRUE=0.499859504821351386

sca "{\"results\":[[$NODE,$TRUE,1e-12]]}"                        "$TMP/explicit.yaml"
sca "{\"velocityfield\":\"\",\"results\":[[$NODE,$TRUE,1e-12]]}"  "$TMP/omitted.yaml"
sca "{\"velocityfield\":\"  VELOCITYFIELD: \\\"function\\\"\n\",\"results\":[[$NODE,$TRUE,1e-12]]}" \
    "$TMP/bogus.yaml"

# The omission has to be real for the rest to mean anything.
...
# Not one word about the missing key, at any severity.
echo "OMITTED_MENTIONS_VELOCITY=$(grep -ci 'velocit' "$TMP/OMITTED.log")"
# The two fabricated diagnostics the entry used to quote are nowhere.
...
# And 4C DOES complain about a genuinely unusable setting, so silence in the
# omitted arm is evidence and not just a quiet build.
grep -m1 -F "Function with index -1 (i.e. input FUNCT-1) not available." "$TMP/BOGUS.log"
grep -m1 -F "4C_utils_function_manager.hpp" "$TMP/BOGUS.log"
exit 0
