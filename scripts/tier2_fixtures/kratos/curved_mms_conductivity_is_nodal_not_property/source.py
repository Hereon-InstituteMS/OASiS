"""Tier-2: the diffusion variable is chosen through the settings object.

The element never reads Properties for conductivity. It asks
the settings object which VARIABLE carries
diffusivity, then reads that variable off the nodes.
...
def main() -> int:
    bad = 0
    if not _probe('KM.CONDUCTIVITY', lambda: (KM.CONDUCTIVITY), True):
        bad += 1
    if not _probe('CDA.CONDUCTIVITY', lambda: (CDA.CONDUCTIVITY), False):
        bad += 1
    if not _probe('KM.HEAT_FLUX', lambda: (KM.HEAT_FLUX), True):
        bad += 1
    if not _probe('KM.REACTION_FLUX', lambda: (KM.REACTION_FLUX), True):
        bad += 1
    if not _probe('settings_SetDiffusionVariable', lambda: (KM.ConvectionDiffusionSettings().SetDiffusionVariable(KM.CONDUCTIVITY) or True), True):
        bad += 1
    if not _probe('settings_GetDiffusionVariable', lambda: ((lambda s: (s.SetDiffusionVariable(KM.CONDUCTIVITY), s.GetDiffusionVariable() == KM.CONDUCTIVITY)[1])(KM.ConvectionDiffusionSettings())), True):
        bad += 1
