"""Tier-2: the process is Dam's; the variable it writes is core's.

The claim's silent-failure mode (rc=0, zero pressure) cannot be
provoked without a full analysis stage. What is checkable is
the API surface the parameter block has to name.
...
def main() -> int:
    bad = 0
    if not _probe('DAM.DamHydroConditionLoadProcess', lambda: (DAM.DamHydroConditionLoadProcess), True):
        bad += 1
    if not _probe('KM.DamHydroConditionLoadProcess', lambda: (KM.DamHydroConditionLoadProcess), False):
        bad += 1
    if not _probe('KM.POSITIVE_FACE_PRESSURE', lambda: (KM.POSITIVE_FACE_PRESSURE), True):
        bad += 1
    if not _probe('DAM.POSITIVE_FACE_PRESSURE', lambda: (DAM.POSITIVE_FACE_PRESSURE), False):
        bad += 1
    if not _probe('SMA.POSITIVE_FACE_PRESSURE', lambda: (SMA.POSITIVE_FACE_PRESSURE), False):
        bad += 1
