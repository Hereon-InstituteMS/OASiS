"""Tier-2: the thermomechanical element's variables live on DamApplication.

The claim's trap is which module owns THERMAL_EXPANSION. It is
Dam's, not StructuralMechanics'. Dotting the
wrong module raises AttributeError before any solve.
...
def main() -> int:
    bad = 0
    if not _probe('DAM.NODAL_REFERENCE_TEMPERATURE', lambda: (DAM.NODAL_REFERENCE_TEMPERATURE), True):
        bad += 1
    if not _probe('SMA.NODAL_REFERENCE_TEMPERATURE', lambda: (SMA.NODAL_REFERENCE_TEMPERATURE), False):
        bad += 1
    if not _probe('DAM.THERMAL_EXPANSION', lambda: (DAM.THERMAL_EXPANSION), True):
        bad += 1
    if not _probe('SMA.THERMAL_EXPANSION', lambda: (SMA.THERMAL_EXPANSION), False):
        bad += 1
    if not _probe('KM.THERMAL_EXPANSION', lambda: (KM.THERMAL_EXPANSION), False):
        bad += 1
