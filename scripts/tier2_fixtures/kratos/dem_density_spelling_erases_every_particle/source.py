"""Tier-2 (kratos.dem::6): DENSITY is accepted, never read, and every particle
silently disappears.

Particle density is PARTICLE_DENSITY. DENSITY is also a real Kratos variable, so
the materials reader accepts it without a word -- and then nothing reads it.
Properties[PARTICLE_DENSITY] returns the inserted default 0.0, particle mass
becomes zero, the integrator divides by it, and the resulting non-finite
coordinates fail the bounding-box test, so every particle is erased.

    materials write DENSITY          -> PARTICLE_DENSITY = 0.0, DENSITY = 2650.0
                                        4 elements at the start, 0 at the end
                                        exit 0, ANALYSIS COMPLETED, no warning
    materials write PARTICLE_DENSITY -> PARTICLE_DENSITY = 2650.0, DENSITY unset
                                        4 elements at the end

Both variables are resolved through KratosGlobals.GetVariable so the probe
cannot accidentally read one module's spelling for the other's.

MUTATION CONTROL (T2_MUTATE=1): the failing deck is written with
PARTICLE_DENSITY, i.e. correctly, so the run that must lose its particles keeps
them. Only the key in the materials file changes.
"""
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="dem_wrongkey_") as d:
        key = "PARTICLE_DENSITY" if MUTATE else "DENSITY"
        if MUTATE:
            print("mutation=materials_written_with_the_correct_PARTICLE_DENSITY")
        _write_deck(d, particle_density_key=key, final_time=2.0e-2)
        wrong, wrong_rc, wrong_log = _run(d)

    with tempfile.TemporaryDirectory(prefix="dem_rightkey_") as d:
        _write_deck(d, final_time=2.0e-2)
        right, right_rc, _ = _run(d)

    checks = [
        ("wrong_key_particle_density_is_zero",
         wrong.get("particle_density"), 0.0),
        ("wrong_key_density_holds_the_value", wrong.get("density"), 2650.0),
        ("wrong_key_loses_every_particle", wrong.get("spheres_elements"), 0),
        ("wrong_key_raises_nothing", wrong.get("exception"), None),
        ("wrong_key_completes", "ANALYSIS COMPLETED" in wrong_log, True),
        ("right_key_particle_density_is_set",
         right.get("particle_density"), 2650.0),
        ("right_key_keeps_every_particle", right.get("spheres_elements"), 4),
        ("both_exit_zero", (wrong_rc, right_rc), (0, 0)),
    ]
    return _report(checks, [], "dem_density_spelling_check",
                   "the DENSITY-vs-PARTICLE_DENSITY claim")
