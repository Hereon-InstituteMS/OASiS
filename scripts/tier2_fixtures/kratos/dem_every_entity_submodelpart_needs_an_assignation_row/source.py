"""Tier-2 (kratos.dem::10): a sub model part carrying entities but missing from
material_assignation_table fails deep in the solver, not at materials-read time.

The wall is the one people forget. With the wall mdpa present and its row left
out of the table, MaterialsAssignationUtility raises nothing at all -- the error
comes later, out of ExplicitSolverStrategy::InitializeFEMElements:

    RuntimeError: Error: PROPERTIES_ID is not set for SubModelPart
    DEM-FEM-Wall_floor . Make sure the Materials file contains material
    assignation for this SubModelPart

CORRECTION TO THE CLAIM AS WRITTEN (measured 2026-08-07): the claim said the
message "names the sub model part by its numeric id, not by name". On this build
it names it BY NAME, exactly as quoted above. The claim text has been amended
rather than the fixture tuned to match it.

MUTATION CONTROL (T2_MUTATE=1): the wall row is put back into the assignation
table, so the deck that must fail no longer does. Only the materials file
changes.
"""
...
def main() -> int:
    table = [["SpheresPart", "g"]]
    if MUTATE:
        print("mutation=wall_row_restored_to_material_assignation_table")
        table = table + [["RigidFacePart.DEM-FEM-Wall_floor", "w"]]

    with tempfile.TemporaryDirectory(prefix="dem_norow_") as d:
        _write_deck(d, walls=True, assignation=table)
        obs, rc, log = _run(d)

    msg = obs.get("message") or ""
    checks = [
        ("raises", obs.get("exception"), "RuntimeError"),
        ("names_properties_id", "PROPERTIES_ID is not set for SubModelPart"
         in msg, True),
        ("names_the_wall_part_by_name", "DEM-FEM-Wall_floor" in msg, True),
        ("advises_the_materials_file",
         "Materials file contains material assignation" in msg, True),
        ("never_reached_finalize", obs.get("reached_finalize"), False),
    ]
    return _report(checks, [("first_message", msg[:110])],
                   "dem_assignation_table_check",
                   "the assignation-table-covers-every-part claim")
