"""Tier-2 (kratos.dem::3): solver_settings.model_import_settings.input_filename
does not choose the DEM mesh.

The key exists, sphere_strategy validates it, and it has no effect: DEM builds
every mesh path from problem_name. A deck whose input_filename points squarely
at a real, present mdpa still reports the file it wanted as missing and runs on
an empty model.

    problem_name "case", file otherDEM.mdpa, input_filename "otherDEM"
        -> 'DEM: Input file DEM.mdpa not found. Continuing.'
        -> 0 elements, exit 0

This claim used to be marked source-read only. It is executed here.

MUTATION CONTROL (T2_MUTATE=1): problem_name is changed to "other" so it agrees
with the file on disk. The mesh is then found by problem_name -- not by
input_filename, which is unchanged throughout -- and the empty model disappears.
"""
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="dem_inert_") as d:
        problem = "other" if MUTATE else "case"
        if MUTATE:
            print("mutation=problem_name_changed_to_agree_with_the_file_on_disk")
        _write_deck(
            d, problem=problem, spheres_stem="otherDEM", walls=False,
            assignation=[["SpheresPart", "g"]],
            params={"solver_settings": {
                "strategy": "sphere_strategy",
                "model_import_settings": {"input_type": "mdpa",
                                          "input_filename": "otherDEM"},
                "material_import_settings": {
                    "materials_filename": "MaterialsDEM.json"}}})
        obs, rc, log = _run(d)

    checks = [
        ("input_filename_did_not_load_the_mesh", obs.get("spheres_elements"), 0),
        ("still_reports_the_problem_name_path",
         "Input file DEM.mdpa not found" in log, True),
        ("raises_nothing", obs.get("exception"), None),
        ("completes", "ANALYSIS COMPLETED" in log, True),
        ("exit_zero", rc, 0),
    ]
    return _report(checks, [("elements", obs.get("spheres_elements"))],
                   "dem_input_filename_inert_check",
                   "the inert-input_filename claim")
