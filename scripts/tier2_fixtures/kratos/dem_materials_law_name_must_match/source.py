"""Tier-2: the DEM law name is matched verbatim.

Two catalog sections carry this claim. Both are pinned so
either one going stale is caught.
...
def main() -> int:
    bad = 0
    if not _probe('DEM_D_Hertz_viscous_Coulomb', lambda: (DEM.DEM_D_Hertz_viscous_Coulomb), True):
        bad += 1
    if not _probe('DEM_D_Linear_viscous_Coulomb', lambda: (DEM.DEM_D_Linear_viscous_Coulomb), True):
        bad += 1
    if not _probe('Hertz_viscous_Coulomb_abbrev', lambda: (DEM.Hertz_viscous_Coulomb), False):
        bad += 1
    if not _probe('DEM_D_Hertz_abbrev', lambda: (DEM.DEM_D_Hertz), False):
        bad += 1
