"""Tier-2 (kratos.dem::7): omitting YOUNG_MODULUS gives zero contact stiffness,
not an error.

The property read inserts 0.0 rather than raising, so the particles become
mutually transparent and the run reports nothing at all.

    materials without YOUNG_MODULUS -> Properties[YOUNG_MODULUS] == 0.0
                                       run completes, exit 0, no warning
    materials with    YOUNG_MODULUS -> Properties[YOUNG_MODULUS] == 1e7

MUTATION CONTROL (T2_MUTATE=1): YOUNG_MODULUS is put back into the deck that is
supposed to be missing it, so the zero read-back becomes 1e7. Only the materials
file changes.
"""
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="dem_noE_") as d:
        omit = not MUTATE
        if MUTATE:
            print("mutation=YOUNG_MODULUS_restored_to_the_deck_that_must_lack_it")
        _write_deck(d, omit_young=omit)
        without, without_rc, without_log = _run(d)

    with tempfile.TemporaryDirectory(prefix="dem_withE_") as d:
        _write_deck(d)
        with_, with_rc, _ = _run(d)

    checks = [
        ("omitted_reads_exactly_zero", without.get("young_modulus"), 0.0),
        ("omitted_raises_nothing", without.get("exception"), None),
        ("omitted_completes", "ANALYSIS COMPLETED" in without_log, True),
        ("omitted_keeps_particles", without.get("spheres_elements"), 4),
        ("present_reads_the_value", with_.get("young_modulus"), 10000000.0),
        ("both_exit_zero", (without_rc, with_rc), (0, 0)),
    ]
    return _report(checks, [], "dem_missing_young_modulus_check",
                   "the missing-YOUNG_MODULUS claim")
