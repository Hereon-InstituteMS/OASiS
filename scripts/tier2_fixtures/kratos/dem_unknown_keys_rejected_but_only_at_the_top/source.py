"""Tier-2 (kratos.dem::11): validation catches a typo at the top level and
inside solver_settings, and misses one nested anywhere else.

ValidateAndAssignDefaults is FLAT. It compares the top-level key set against the
DEM defaults, and sphere_strategy separately compares solver_settings against
its own four-key default -- but neither descends into any other sub-block.

    top level   "MaxTimestep" (lowercase s)  -> RuntimeError, 'is present in
                                                this Parameters but NOT in the
                                                default values'
    solver_settings "not_a_key"              -> the same error
    nested in BoundingBoxServoLoadingSettings
                "not_a_key_either"           -> accepted, run completes, exit 0

The third probe is the one worth having: it is the half that bites, because a
misspelling there is indistinguishable from a working deck.

MUTATION CONTROL (T2_MUTATE=1): all three typos are corrected -- MaxTimestep
becomes MaxTimeStep, and the two junk keys are dropped -- so the two decks that
must be rejected are accepted and the third is unchanged. Only the decks change.
"""
...
def main() -> int:
    if MUTATE:
        print("mutation=all_three_typos_corrected_in_the_decks")

    with tempfile.TemporaryDirectory(prefix="dem_typo1_") as d:
        _write_deck(d, params=({"MaxTimeStep": 2e-5} if MUTATE
                               else {"MaxTimestep": 2e-5}))
        top, _, _ = _run(d)

    with tempfile.TemporaryDirectory(prefix="dem_typo2_") as d:
        ss = {"strategy": "sphere_strategy",
              "material_import_settings": {
                  "materials_filename": "MaterialsDEM.json"}}
        if not MUTATE:
            ss["not_a_key"] = 1
        _write_deck(d, params={"solver_settings": ss})
        solver, _, _ = _run(d)

    with tempfile.TemporaryDirectory(prefix="dem_typo3_") as d:
        nested = {"BoundingBoxServoLoadingType": "isotropic"}
        if not MUTATE:
            nested["not_a_key_either"] = 1
        _write_deck(d, params={"BoundingBoxServoLoadingSettings": nested})
        deep, deep_rc, deep_log = _run(d)

    top_msg = top.get("message") or ""
    solver_msg = solver.get("message") or ""
    checks = [
        ("toplevel_typo_raises", top.get("exception"), "RuntimeError"),
        ("toplevel_typo_names_the_key", 'name "MaxTimestep"' in top_msg, True),
        ("toplevel_typo_says_not_in_defaults",
         "NOT in the default values" in top_msg, True),
        ("solver_settings_typo_raises", solver.get("exception"), "RuntimeError"),
        ("solver_settings_typo_says_not_in_defaults",
         "NOT in the default values" in solver_msg, True),
        ("nested_typo_is_ignored", deep.get("exception"), None),
        ("nested_typo_run_completes", "ANALYSIS COMPLETED" in deep_log, True),
        ("nested_typo_exit_zero", deep_rc, 0),
    ]
    return _report(checks, [], "dem_validation_is_flat_check",
                   "the flat-validation claim")
