"""Tier-2 (kratos.dem::2): a missing wall mdpa is an INFO line, and the
particles then fall through a boundary the user believes exists.

Same silence as the spheres file, but this half is the dangerous one: the run
succeeds, the particles are all still there, and the only difference is that
RigidFacePart holds nothing.

    wall file present  ->  2 conditions
    wall file absent   ->  0 conditions, 4 particles, exit 0, ANALYSIS COMPLETED
                           'DEM: Input file DEM_FEM_boundary.mdpa not found.'

MEASURED CAVEAT, recorded because it is easy to get wrong: the silence only
happens if nothing else refers to the wall. With the wall row still in
material_assignation_table but the file gone, the run does NOT continue -- it
raises 'There is no sub model part with name "DEM-FEM-Wall_floor" in model part
"RigidFacePart"'. The deck under test therefore omits both together, which is
what a user who never created a wall would have.

MUTATION CONTROL (T2_MUTATE=1): the wall mdpa is written for the second deck
too, so the run that must come back with no wall comes back with two conditions.
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="dem_wall_") as d:
        _write_deck(d, walls=True)
        withw, withw_rc, _ = _run(d)

    with tempfile.TemporaryDirectory(prefix="dem_nowall_") as d:
        walls = True if MUTATE else False
        if MUTATE:
            print("mutation=wall_mdpa_written_for_the_deck_that_must_lack_one")
        _write_deck(d, walls=walls, assignation=[["SpheresPart", "g"]])
        nowall, nowall_rc, nowall_log = _run(d)

    checks = [
        ("wall_present_reads_conditions", withw.get("wall_conditions"), 2),
        ("wall_absent_zero_conditions", nowall.get("wall_conditions"), 0),
        ("wall_absent_keeps_particles", nowall.get("spheres_elements"), 4),
        ("wall_absent_raises_nothing", nowall.get("exception"), None),
        ("wall_absent_logs_not_found",
         "Input file DEM_FEM_boundary.mdpa not found" in nowall_log, True),
        ("wall_absent_completes", "ANALYSIS COMPLETED" in nowall_log, True),
        ("both_exit_zero", (withw_rc, nowall_rc), (0, 0)),
    ]
    return _report(checks, [], "dem_wall_optional_check",
                   "the silently-optional wall claim")
