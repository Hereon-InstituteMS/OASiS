"""Tier-2: VELOCITY and PRESSURE are both core and both assignable.

The reversed configuration is not rejected by anything: both
variables exist and both can be fixed on any
boundary. The mistake shows up only in the answer.
...
def main() -> int:
    bad = 0
    if not _probe('KM.VELOCITY', lambda: (KM.VELOCITY), True):
        bad += 1
    if not _probe('KM.PRESSURE', lambda: (KM.PRESSURE), True):
        bad += 1
    if not _probe('FDA.VELOCITY', lambda: (FDA.VELOCITY), False):
        bad += 1
    if not _probe('FDA.PRESSURE', lambda: (FDA.PRESSURE), False):
        bad += 1
