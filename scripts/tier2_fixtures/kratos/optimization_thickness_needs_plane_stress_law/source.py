"""Tier-2: both laws exist; only the plane-stress one scales by THICKNESS.

A thickness gradient under a plane-strain law is zero-effect,
with no error. The fixture pins that both laws are real and
that the variables the claim requires are core.
...
def main() -> int:
    bad = 0
    if not _probe('SMA.LinearElasticPlaneStress2DLaw', lambda: (SMA.LinearElasticPlaneStress2DLaw()), True):
        bad += 1
    if not _probe('SMA.LinearElasticPlaneStrain2DLaw', lambda: (SMA.LinearElasticPlaneStrain2DLaw()), True):
        bad += 1
    if not _probe('KM.THICKNESS', lambda: (KM.THICKNESS), True):
        bad += 1
    if not _probe('KM.DENSITY', lambda: (KM.DENSITY), True):
        bad += 1
    if not _probe('laws_are_different_classes', lambda: (type(SMA.LinearElasticPlaneStress2DLaw()) is not type(SMA.LinearElasticPlaneStrain2DLaw())), True):
        bad += 1
    if not _probe('laws_are_the_same_class', lambda: (type(SMA.LinearElasticPlaneStress2DLaw()) is type(SMA.LinearElasticPlaneStrain2DLaw())), False):
        bad += 1
    if not _probe('SMA.THICKNESS', lambda: (SMA.THICKNESS), False):
        bad += 1
