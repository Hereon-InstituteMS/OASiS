"""Tier-2: TANGENT_OPERATOR_ESTIMATION is a CLA Properties integer.

The claim's three regimes only show up in a full nonlinear
solve. What is checkable is that the knob is real,
which module owns it, and that Properties accepts the
recommended value.
...
def main() -> int:
    bad = 0
    if not _probe('CLA.TANGENT_OPERATOR_ESTIMATION', lambda: (CLA.TANGENT_OPERATOR_ESTIMATION), True):
        bad += 1
    if not _probe('KM.TANGENT_OPERATOR_ESTIMATION', lambda: (KM.TANGENT_OPERATOR_ESTIMATION), False):
        bad += 1
    if not _probe('properties_roundtrip_secant_3', lambda: ((lambda p: (p.SetValue(CLA.TANGENT_OPERATOR_ESTIMATION, 3), p.GetValue(CLA.TANGENT_OPERATOR_ESTIMATION) == 3)[1])(KM.Model().CreateModelPart('to').CreateNewProperties(1))), True):
        bad += 1
