"""Tier-2: vtk_output_process and KM.VtkOutput are the real output surface.

A JSON output block names a python module and a Factory. The
fixture pins that both exist under the spelling the
claim gives, on core rather than on the application.
...
import KratosMultiphysics.vtk_output_process as vtkproc
...
def main() -> int:
    bad = 0
    if not _probe('vtk_output_process_Factory', lambda: (vtkproc.Factory), True):
        bad += 1
    if not _probe('KM.VtkOutput', lambda: (KM.VtkOutput), True):
        bad += 1
    if not _probe('CDA.VtkOutput', lambda: (CDA.VtkOutput), False):
        bad += 1
