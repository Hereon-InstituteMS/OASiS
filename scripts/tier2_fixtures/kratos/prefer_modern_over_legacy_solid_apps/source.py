"""Tier-2: the legacy solid/contact applications are unreachable on a pip stack.

A reader could read this as a style preference. It is not:
the legacy applications cannot be imported at all here, so
there is no older path to fall back to.
...
CASES = [('StructuralMechanicsApplication', True), ('ContactStructuralMechanicsApplication', True), ('SolidMechanicsApplication', False), ('ContactMechanicsApplication', False)]
if MUTATE:
