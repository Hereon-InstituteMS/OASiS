"""Tier-2: RANS depends on FluidDynamics and both are present.

A dependency claim, checkable by import. The order matters:
RANS cannot load without FluidDynamics, so a stack that has one
and not the other fails inside the loader stub.
"""
...
CASES = [('FluidDynamicsApplication', True), ('RANSApplication', True), ('NoSuchKratosApplication', False)]
