...
_SILENT_WRONG = [
    ...
    "same number, export opposite numbers. Getting only the export wrong fails "
    "a coupling that is physically right; getting only the applied BC wrong "
    "drives the interface the wrong way. "
    "Signal: the run converges and the conservation check still reports "
    "`Interface flux NOT balanced` with the two net magnitudes equal to within "
    "the tolerance and their SIGNS AGREEING — that combination is the "
    "fingerprint, and the check names it: 'The two magnitudes match but the "
    "signs agree, which is the signature of a SIGN-CONVENTION error rather "
    "than a conservation error'. Magnitudes that do NOT match are a different "
    "entry. If instead the APPLIED sign is flipped, nothing complains about "
    "signs at all: the residual grows from the first iteration and the "
    "exported interface values alternate in sign. "
    ...
    "matches, the sensitivity probe reports a healthy response, and the "
    "verdict comes back trustworthy — the answer is simply in the wrong units "
    "and no check in OASiS can see it. Only an independent answer separates "
    ...
    "and it NARROWS as the split gets more unbalanced. theta=1.0 is not a "
