...
CRITICAL_KNOWLEDGE: dict = {
    ...
    "step_1_debug_vs_release_decides_what_you_will_see": {
        ...
        "how_to_get_the_debug_diagnostics": [
            ...
            "-DDEBUG to YOUR translation unit re-activates the Asserts "
            "that live in deal.II HEADERS (templates and inline "
            "functions — SparseMatrix, FEValues, SolverCG, "
            "FEEvaluation, ...). It cannot re-activate Asserts that "
            "live in the compiled library (DoFHandler::distribute_dofs "
            "and most of source/*.cc); those misuses still segfault. "
            "Verified: an out-of-pattern SparseMatrix::add() silently "
            "drops the value on Release and aborts with the full "
            "'You are trying to access the matrix entry with index "
            "<i,j>, but this entry does not exist in the sparsity "
            "pattern of this matrix.' report once -DDEBUG is added.",
        ],
    },
