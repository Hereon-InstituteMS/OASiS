    ...
    "matrix_free": {
        ...
        "pitfalls": [
            (
                ...
                "elements ABORTS the process — it does not "
                "silently disable vectorization, and it does NOT "
                "throw a catchable exception. The failure comes "
                "from DEAL_II_NOT_IMPLEMENTED() at "
                "shape_info.templates.h:136, inside internal::"
                "MatrixFreeFunctions::get_element_type_specific_"
                "information. That macro aborts rather than "
                "throwing, so wrapping reinit in try/catch "
                "(const std::exception&) does NOT catch it: the "
                "process dies with SIGABRT (rc=134) after printing "
                "a deal.II banner plus stacktrace. The banner text "
                "is the generic \"You are trying to use "
                ...
                "quote. It is not gated on NDEBUG, so it fires on "
                "a Release build too. Measured on deal.II "
                "9.8.0-pre 2026-08-03: FE_Q(2) reinit OK "
                "(8 cell batches, 2 active lanes in batch 0); "
                "FE_RaviartThomas(1) aborts, rc=134, uncaught by "
                "an enclosing catch(const std::exception&). Guard "
                "by checking the element type BEFORE reinit; you "
                "cannot recover from this at run time. "
