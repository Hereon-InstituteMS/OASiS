...
KNOWLEDGE = {
    ...
    "pitfalls": [
        ...
        "(c) When the flow has CLOSED CHARACTERISTICS (a rotational "
        "beta on a box, where interior streamlines never touch an "
        "inflow boundary) no downstream ordering exists, the pure "
        "transport operator is close to singular, and every "
        "preconditioner in this list degrades — several exhaust a "
        "5000-iteration budget. That is a well-posedness problem, "
        "not a preconditioner problem: add reaction/diffusion or "
        "change the boundary data. "
        "(d) Scalar (point) SSOR is NOT reliably poor here, contrary "
