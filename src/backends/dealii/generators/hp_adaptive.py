...
KNOWLEDGE = {
    ...
    "pitfalls": [
        ...
        "nothing. Signal: the program SEGFAULTS (rc=139) with no "
        "message at all on this Release (NDEBUG) install — NEITHER "
        "an ExcMessage nor an n_dofs()==0. Measured on deal.II "
        "9.8.0-pre 2026-08-03: a "
        "2-entry hp::FECollection, one cell given "
        "set_active_fe_index(5), then distribute_dofs(collection) "
        "-> rc=139 (SIGSEGV) with no message at all. The range "
        "check is an Assert and is compiled out. Guard it "
        "yourself: assert every index you set is < "
        "fe_collection.size() before calling distribute_dofs. "
        "(This entry used to promise ExcMessage('Index in "
        "FECollection out of range').)",
