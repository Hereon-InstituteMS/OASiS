...
KNOWLEDGE = {
    ...
    "pitfalls": [
        ...
        "Signal: `GridTools::get_boundary_ids(tria)` returns `{0}` "
        ...
        "GridTools::get_boundary_ids(tria) after creating the mesh. "
        "Signal: get_boundary_ids(tria) returns `{0}`, not the "
        ...
        "(unit-square MMS u=sin(pi x)sin(pi y), 6 adaptive cycles): "
        "without constraints max|A_ij-A_ji|/max|A| = 0 at every "
        "cycle and CG converged in 8/11/16/26/47/89 steps — the "
        "SAME counts as the constrained run — while the L2 error "
        "GREW 3.04e-2 -> 6.74e-2; with constraints it fell "
        "3.04e-2 -> 1.35e-2. Signal: an L2 error that stops "
        "decreasing (or increases) under refinement while the "
        "solver looks healthy. (This entry previously claimed a "
