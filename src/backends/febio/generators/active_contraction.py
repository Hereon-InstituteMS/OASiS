    ...
    "active_contraction": {
        ...
        "pitfalls": [
            (
                ...
                "Signal: NOT a parse error — the deck reads `...SUCCESS!` and then fails with `8 negative jacobians detected.` (the count is the number of inverted integration points), `------- failed to converge at time : <t>`, `Number of time steps completed .... : 0` and exit 1. Executed. Because it is an element-inversion failure and not a schema failure, no message names the material or the missing passive solid — treat a negative-jacobian failure at step 1 in an active model as a missing passive base first. "
