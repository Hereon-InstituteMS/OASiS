...
KNOWLEDGE = {
    "biphasic": {
        ...
        "pitfalls": [
            ...
            "Signal: [FALSIFIED 2026-08-03 — neither 'unknown "
            "material type' nor 'invalid module' occurs anywhere in "
            "the FEBio 4.12 binary or its libraries, and an "
            "unregistered <Module type> does not produce a message "
            "at ALL: it SEGFAULTS (exit 139) with stdout truncated "
            "mid-line at `Reading file ...`. An unregistered "
            "<material type> is rejected on STDOUT with `tag "
            "\"material\" (line N) : invalid value for attribute "
            "\"type\"`. Original unverified text follows] stderr "
            "contains 'unknown material type' or "
            ...
            "are not valid. "
            "Signal: FALSIFIED 2026-08-03 — the string 'invalid "
            "material parameter' does not occur anywhere in the "
            "FEBio 4.12 binary or its libraries. An unrecognised "
            "material parameter is rejected on stdout with `tag "
            "\"<param>\" (line N) : unrecognized tag` and `Reading "
            "file ...FAILED!` (executed: `<nu>` inside `isotropic "
            "elastic`, `<Y0>` inside `von-Mises plasticity`).",
            ...
            "FURTHER CORRECTION 2026-08-03: 'that leaves bicgstab (the real "
            "answer)' is too strong. Re-running the same 20-solver sweep on "
