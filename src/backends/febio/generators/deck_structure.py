...
DECK_KNOWLEDGE = {
    ...
    "linear_solvers": {
        ...
        "null_test_solver": (
            ...
            "solved nothing: on the one-element biphasic deck of "
            "scripts/tier2_fixtures/febio/"
            "null_test_linear_solver_reports_success it reported "
            "sz = -2.00200400802 against the correct "
            "-3.60750105073, i.e. 44.5% low. "
            "CORRECTED 2026-08-03: this entry previously quoted "
            "-2.00691787274 as the reference and called the null "
            "result 'close enough to look plausible'. Both are "
            "wrong — the fixture itself prints "
            "`relative_sz_difference=0.445044`. The value the null "
            "solver returns is NOT a near-miss; it is whatever the "
            "prescribed BCs alone impose, with every free degree "
            "of freedom left at zero. On the same deck the fluid "
            "pressure, the entire point of a biphasic run, comes "
            "back identically 0 at every node. Do not rely on the "
            "answer merely LOOKING wrong, though: the deck-to-deck "
            "size of the error is arbitrary, and on a solid deck "
            "with prescribed uz = -0.1 the null solver reproduces "
            "uz exactly and only silently drops the lateral "
            "Poisson response (ux = uy = 0 instead of "
            "+/-0.0201933221838). (Live-verified 2026-08-03.)"),
    },
