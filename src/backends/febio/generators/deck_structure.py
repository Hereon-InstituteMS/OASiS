...
DECK_KNOWLEDGE = {
    ...
    "how_to_enumerate_features_yourself": (
        ...
        "1428 factories: 159 FEMATERIAL_ID, 288 FEPLOTDATA_ID, "
    ...
    "section_order": (
        ...
        "live, the hard constraints are exactly three: "
        ...
        "attribute \"mat\"`; "
        "(3) <Mesh> must precede <MeshDomains> and <Boundary>, "
        "else `tag \"bc\" (line N) : invalid value for attribute "
        "\"node_set\"`. "
    ...
    "linear_solvers": {
        ...
        "unsymmetric": (
            ...
            "format. Verified live on the USE_MKL=OFF build: of the "
            "20 registered solvers only `bicgstab` accepts it. "
            "skyline / fgmres / cg / schur / bipn / superlu_mt / "
            "accelerate / diagonal / hypre_gmres / pardiso-project / "
            "strategy all abort with `The selected linear solver "
            "does not support the requested matrix format.`, `LU` "
            "aborts with `Linear solver failed to find solution`, "
            "`ilu0` with `Fatal error in factorization of stiffness "
            "matrix`, `block` with `An error occurred during "
            "preprocessing of linear solver`."),
    },
    ...
    "output_naming": {
        ...
        "log_variables": (
            ...
            "(Green-Lagrange), ex..exz (Euler-Almansi), Ux..Uxz "
    ...
    "shipped_template_execution_status": (
        ...
        "eight are rejected for a <material> without a name= "
