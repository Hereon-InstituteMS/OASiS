    ...
    "multiphasic": {
        ...
        "pitfalls": [
            (
                ...
                "<solute sol=N>. Signal: CORRECTED 2026-08-03 by "
                "execution on FEBio 4.12.0 — the failure is NOT at "
                "input-parse time. "
                "A multiphasic material whose <solute sol=\"1\"> is "
                "not backed by a <Globals><Solutes> declaration "
                "READS SUCCESSFULLY and fails at model "
                "initialisation with `no match with global solute "
                "data` / `Failed initializing material 1 (name=\"...\")` / "
                "`Model initialization failed`, exit 1 — so a "
                "wrapper that only checks the `Reading file "
                "...SUCCESS!` line will believe the deck is fine. "
                "Note also that `multiphasic` requires an "
                "<osmotic_coefficient> property BEFORE the solute "
                "check is reached (`Component \"<name>\" needs to "
                "have property \"osmotic_coefficient\" defined`). "
                "Neither `unknown solute reference` nor "
                "`Globals/Solutes missing` — the two strings this "
                "entry quoted before — occurs anywhere in the 4.12 "
                "binary or its libraries."
            ),
            (
                ...
                "the input. Signal: CORRECTED 2026-08-03 by execution "
                "on FEBio 4.12.0 — a <solute> child inside a `biphasic` "
                "material is rejected on STDOUT, in the standard "
                "ERROR box, with `tag \"solute\" (line N) : "
                "unrecognized tag` and `Reading file ...FAILED!` — "
                "not on stderr. Neither `unknown material child` nor "
                "`biphasic does not support solutes` occurs anywhere "
                "in the 4.12 binary or its libraries."
            ),
            (
            ...
            (
                ...
                "[FALSIFIED 2026-08-03: this message text does not occur "
                "anywhere in the FEBio 4.12.0.86045466d binary or any of "
                "its shared libraries (`strings` over febio4 + all 12 "
                ".so files, 267541 strings), so this Signal can never "
                "match on 4.12. The physics reasoning is desk research "
                "and was NOT executed] `prescribed concentration without "
                "sol= defaults to "
                "1`. (Audit 2026-06-02.)"
            ),
        ],
