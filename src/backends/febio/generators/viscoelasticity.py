    ...
    "viscoelasticity": {
        ...
        "pitfalls": [
            (
                ...
                "an error. Signal: CORRECTED 2026-08-03 by execution on "
                "FEBio 4.12.0 — the physics claim HOLDS but the "
                "quoted message did not. A COUPLED child "
                "(`neo-Hookean` or `isotropic elastic`) is rejected "
                "at parse with `Component \"<matname>\" needs to "
                "have property \"elastic\" defined (line N)` and "
                "`Reading file ...FAILED!`: FEBio refuses the "
                "coupled material as the `elastic` property, so the "
                "message reads as a MISSING property, not a "
                "mismatch. The previously quoted `uncoupled "
                "viscoelastic requires uncoupled ground-state "
                "material` does not occur anywhere in the 4.12 "
                "binary or its libraries. Verified working: "
                "<material type=\"uncoupled viscoelastic\">"
                "<g1>0.5</g1><t1>1.0</t1><density>1</density>"
                "<elastic type=\"Mooney-Rivlin\"><density>1</density>"
                "<c1>1</c1><c2>0</c2><k>1000</k></elastic></material> "
                "runs to N O R M A L   T E R M I N A T I O N."
            ),
            (
