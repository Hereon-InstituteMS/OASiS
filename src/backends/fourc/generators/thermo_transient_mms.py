        ...
        return {
            ...
            "time_integration": {
                "OneStepTheta": (
                    ...
                    "genuinely 2nd-order in time and theta=1 is "
                    "1st-order. Exercised by dt-halving on this build "
                    "(2026-08-01). Grade theta=0.5 from Richardson "
                    "differences rather than from an error-vs-exact "
                    "table: on a fixed mesh the latter saturates at "
                    "the spatial floor and understates the order."
                ),
            },
            "materials": {
                "MAT_Fourier": {
            ...
            "pitfalls": [
                ...
                "the MMS field, while a silently dropped source "
                "leaves an O(1) error that no dt refinement removes. "
                "(Verified 2026-08-01.)",
                ...
                "flatten however good the time integrator is — the "
                "temporal error has simply fallen below a spatial "
                "error that dt cannot touch. Grade the temporal "
                "order from Richardson differences "
                "||u_dt - u_dt/2|| of consecutive-dt solutions on "
                "the SAME mesh instead: the spatial error is "
                "identical in both and cancels in the difference. "
                "Signal: an error-vs-exact dt series whose "
                "successive ratios decay toward 1 while the "
                "Richardson ratios on the SAME runs hold near 2^p "
                "is the spatial floor, not a solver defect; the two "
                "diagnostics disagreeing is itself the tell. "
                "(Verified 2026-08-01.)",
