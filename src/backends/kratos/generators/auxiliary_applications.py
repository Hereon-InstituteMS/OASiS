...
KNOWLEDGE = {
    "_auxiliary_overview": {
        ...
        "pitfalls": [
            ...
            "[API] DEM is always 3D internally: the MDPA must reference SphericParticle3D even for problems posed in a 2D plane. There is NO SphericParticle2D element registered; attempting ModelPart.CreateNewElement(\"SphericParticle2D\", ...) raises RuntimeError \"Element \\\"SphericParticle2D\\\" is not registered!\" listing the available DEM element types. Signal: RuntimeError 'Element \"SphericParticle2D\" is not registered!' or 'Maybe you need to import the application where it is defined?' when an agent generates a 2D DEM template with the literal 2D element name. (Verified empirically 2026-06-01.)",
