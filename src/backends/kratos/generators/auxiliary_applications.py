...
KNOWLEDGE = {
    "_auxiliary_overview": {
        ...
        "pitfalls": [
                        ...
                        '[Integration] KratosContactStructuralMechanicsApplication is NOT published on PyPI (same gap as TrilinosApplication and DelaunayMeshingApplication). `pip install KratosContactStructuralMechanicsApplication` returns "Could not find a version that satisfies the requirement". To use Contact physics with the modern app the user must build Kratos from source with `-DCONTACT_STRUCTURAL_MECHANICS_APPLICATION=ON` or use the older ContactMechanicsApplication (deprecated in 10.x). '
                        "Signal: pip install reports 'ERROR: Could not find a version that satisfies the requirement KratosContactStructuralMechanicsApplication'; python import raises ModuleNotFoundError 'No module named KratosMultiphysics.ContactStructuralMechanicsApplication'. (Verified empirically 2026-06-01 — pip-only installs cannot use Contact physics; needs full source build.)",
                    ],
    },
}
