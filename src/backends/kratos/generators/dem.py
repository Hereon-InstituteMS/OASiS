...
KNOWLEDGE = {
    "dem": {
        ...
        "pitfalls": [
            ...
            "[Input] Every sub model part that carries entities needs a row in material_assignation_table, including the wall part. Omitting the wall row while the wall mdpa is present is a hard error raised deep in the solver, not at materials-reading time. Signal: RuntimeError 'Error: PROPERTIES_ID is not set for SubModelPart DEM-FEM-Wall_floor . Make sure the Materials file contains material assignation for this SubModelPart' from explicit_solver_strategy InitializeFEMElements. The wording that follows 'SubModelPart' is whatever ModelPart::Name returns for the offending part, so it is the sub model part's NAME when the mdpa gives it one; an earlier version of this entry said the message names it by numeric id instead, and that was corrected on re-measurement. (Verified by execution 2026-08-07; fixture dem_every_entity_submodelpart_needs_an_assignation_row.)",
            ...
            "[Output] The core vtk_output_process cannot be attached to a DEM SpheresPart, so 'DEM writes VTK through the core KM.VtkOutput class' is not something a user can act on: adding a VtkOutputProcess block to output_processes fails during Initialize, before a single step. Signal: output_control_type 'step' raises RuntimeError 'Error: STEP not found in process info of SpheresPart.' and 'time' raises 'Error: TIME not found in process info of SpheresPart.', both from OutputController::Check() at kratos/controllers/output_controller.cpp:81. The cause is ordering, not absence — DEM_analysis_stage.Initialize does its own work and only then calls AnalysisStage.Initialize, whose Check() runs before that model part's ProcessInfo has been populated; probing the same ProcessInfo after Initialize returns shows TIME present (STEP still absent). The DEM-native alternative is not a workaround on a stock install either: DEMApplication.dem_vtk_output opens with 'from pyevtk import hl' and pyevtk is neither a Kratos dependency nor shipped with it, so post_vtk_option: true raises ModuleNotFoundError('pyevtk') from SetGraphicalOutput. What does work is post_gid_option: true, which writes <problem_name>_Post_Files. (Verified by execution 2026-08-07; replaces a 2026-06-01 entry that quoted an unsupported-geometry message never reproduced against DEM. Fixture dem_vtk_output_process_cannot_attach_to_spherespart.)",
        ],
        "guidance": [
