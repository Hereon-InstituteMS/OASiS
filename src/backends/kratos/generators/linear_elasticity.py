...
KNOWLEDGE = {
    "linear_elasticity": {
        ...
        "pitfalls": [
                        ...
                        '[API] For POINT_LOAD on NODES: use AssignVectorVariableProcess with constrained: [false, false, false]. There is no AssignVectorByDirectionProcess class on StructuralMechanicsApplication, but there IS a core Python process module KratosMultiphysics.assign_vector_by_direction_process — and it genuinely fails on load variables because it tries to fix/free the DOF. '
                        "Signal: two different failures depending on the route taken. (a) SMA.AssignVectorByDirectionProcess -> AttributeError (hasattr is False). (b) KratosMultiphysics.assign_vector_by_direction_process.Factory(...) with variable_name POINT_LOAD -> RuntimeError 'Error: Trying to fix/free dof of variable POINT_LOAD_X but this dof does not exist in node #1!'. assign_vector_variable_process on the same model part sets POINT_LOAD = [0, -100, 0] cleanly. For loads carried by CONDITIONS use assign_vector_by_direction_to_condition_process, which works. (Verified by execution 2026-08-03 on Kratos 10.4.0 — supersedes the 2026-06-01 note, which concluded from route (a) alone that 'the named class is not available to crash'; route (b) does crash, and with the originally-documented message.)",
