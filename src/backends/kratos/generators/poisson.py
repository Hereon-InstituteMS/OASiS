...
KNOWLEDGE = {
    "poisson": {
        ...
        "pitfalls": [
                        ...
                        'max T = 0.9755 (exact 1.0), while EulerianConvDiff2D3N '
                        'returns a field with NO diffusion in it. WHAT that wrong '
                        'field looks like depends on DELTA_TIME, so do NOT use '
                        '"all zeros" as the test: with DELTA_TIME = 1.0 the same case '
                        'gives max T = 19.7392 = 2*pi^2, i.e. T reproduces the SOURCE '
                        'field (LHS = M/dt, so T = M^-1 M f = f); with DELTA_TIME = 0 '
                        '(easy to hit by setting ProcessInfo[TIME] and then calling '
                        'CloneTimeStep with the same value) the system degenerates, '
                        'Kratos prints "[WARNING] ResidualBasedBlockBuilderAndSolver: '
                        'ATTENTION! setting the RHS to zero!" and T comes out 0.0 '
                        'everywhere. Use EulerianConvDiff only inside a TRANSIENT '
                        'convection-diffusion solver that supplies the time-integration '
                        'context it expects. '
                        'Signal: check the ELEMENT, not the field — '
                        'CalculateLocalSystem on one triangle returns the consistent '
                        ...
                        'was ALSO wrong: it reported a DELTA_TIME = 0 artifact of its own '
                        'fixture, and a warning IS printed.)',
