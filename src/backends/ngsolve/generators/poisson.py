...
KNOWLEDGE = {
    "poisson": {
        ...
        "pitfalls": [
            ...
            "order 3 upward the vertex/edge/bubble coefficients "
            ...
            "everywhere. On order 1-2 for these expressions the "
            "two still coincide, which is why the trap survives "
            "casual testing. Use Integrate/gfu(mesh(x,y)) sampling "
            "for a true L_inf. (Verified empirically 2026-08-03 "
            "on NGSolve 6.2.2604 — counterexample supplied; the "
            "prior 'stronger counterexample needed' caveat is "
            "resolved.)",
            ...
            "subdivision=N: the exported cells are all LINEAR "
            "VTK triangles, so subdivision is the only way the "
            "P2+ enrichment reaches the file. Signal: on "
            "unit_square maxh=0.3 (24 triangles), a P2 "
            "GridFunction written with subdivision=0 yields a "
            ".vtu with NumberOfCells=24 — exactly one linear cell "
            "per element, so the inter-vertex P2 enrichment is "
            "absent; subdivision=2 yields NumberOfCells=240. "
            "(Verified empirically 2026-08-03 by parsing the "
            "NumberOfCells attribute of the emitted .vtu.)",
