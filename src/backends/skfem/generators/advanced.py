    ...
    "navier_stokes": {
        ...
        "pitfalls": [
            ...
            (
                ...
                "scipy.sparse.linalg.spsolve does NOT raise — it "
                "emits `MatrixRankWarning: Matrix is exactly "
                "singular` (that is the exact wording; 'matrix is "
                "singular' in lower case with no 'exactly' is not "
                "what scipy prints) and RETURNS, giving an "
                "all-NaN vector, or else returns a pressure field "
                "with an arbitrary constant offset. Catch it with "
                "warnings.catch_warnings(record=True) plus an "
                "np.isfinite(x).all() assertion, never with "
                "try/except; pinning p at one DOF removes the "
                "warning. (Quoted string re-checked live "
                "2026-08-06 on scipy 1.15.3 and corrected.)"
            ),
            (
    ...
    "helmholtz": {
        ...
        "pitfalls": [
            (
                ...
                "a real number, not complex').) The SHIPPED "
    ...
    "reaction_diffusion": {
        ...
        "pitfalls": [
            ...
            (
                ...
                "and it sits below 10 for the usual Schnakenberg "
                "parameters — so a ratio that fails is not "
                "evidence the parameters are wrong, and a ratio "
                "just under 10 may still pattern. Compute the "
                "root for your own (a, b) rather than trusting "
                "the number. Signal: at d_v = d_u the first "
            ...
            (
                ...
                "gamma*[k_-^2, k_+^2], whose endpoints are the "
                "two roots of the dispersion quadratic "
                "(d*f_u + g_v)^2 = 4*d*det(J) evaluated at the "
                "homogeneous steady state. Compute those roots "
                ...
                "inequality but leave k_min^2 below "
                "gamma*k_-^2, max(u) - min(u) decays "
