...
KNOWLEDGE = {
    "heat": {
        ...
        "pitfalls": [
            ...
            "the rank-deficient null space. Signal (verified "
            "2026-06-01 with MeshTri refined×3, P1 basis, "
            "f=ones): spsolve returns a finite array but "
            "np.max(np.abs(u)) is order 1e15-1e17, and the "
            "boundary DOF values do not match boundary_values. "
            "Original catalog text said spsolve 'raises singular "
            "matrix' — that is wrong for scipy's SuperLU default; "
