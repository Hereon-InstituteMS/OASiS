...
_FENICS_KNOWLEDGE = {'element_catalog': {'description': 'Complete catalog of finite element families '
 ...
 'navier_stokes': {'description': 'Incompressible Navier-Stokes: rho*(du/dt + '
                   ...
                   'pitfalls': ['[API] Never wrap a dolfinx.fem.petsc.NonlinearProblem '
                                ...
                                'instead. Doubling the mesh resolution makes the '
                                'cold-start blow-up larger, not smaller. Stepping Re '
 ...
 'hyperelasticity': {'description': 'Nonlinear hyperelasticity with large '
                     ...
                     'pitfalls': ['[API] Several signals below name '
                                  ...
                                  'well-conditioned tangent. Signal: dolfinx '
                                  'fem.assemble_scalar of the post-processed pressure '
                                  '(= dU/dJ) gives a bounded value; without the split, '
                                  'the discrete pressure Function at Gauss points '
                                  'oscillates wildly element-to-element. (Claim '
                                  'inherited.)',
 ...
 'thermal_structural': {'description': 'One-way (sequential) thermo-elasticity: first '
                        ...
                        'pitfalls': ['[Numerical] The thermal term is the ONLY place '
                                     ...
                                     'constraint set the deviatoric stress is '
                                     'bit-identical with T_ref = 0 and with T_ref = '
                                     ...
                                     'factor. The often-repeated claim that this makes '
 ...
 'biharmonic': {'description': 'Biharmonic equation (4th order): laplacian^2(u) = f. '
                ...
                'pitfalls': ['[Numerical] Penalty parameter alpha must be large enough '
                             ...
                             'facet visited from both sides). Signal: assembly time '
                             'per step in a biharmonic problem is 5-10x the equivalent '
                             'Poisson; profile shows dolfinx.fem.assemble_matrix '
                             'spending most time in facet kernels. Mixed method (u + '
                             'auxiliary sigma) avoids dS at the cost of doubling the '
                             'DOF count. (Audit 2026-06-02.)',
 ...
 'helmholtz': {'description': 'Helmholtz equation: -laplacian(u) - k^2*u = f. '
               ...
               'pitfalls': ["[Numerical] Need a fine mesh, and '~10 points per "
                            ...
                            'preconditioners (shifted Laplacian). Signal: PETSc GMRES '
                            'with default ILU/Jacobi PC on a k > 100 problem stagnates '
                            'at residual ~1e-2 after 1000 iterations; the '
                            'shifted-Laplacian preconditioner (PC with k_shift = k + '
                            'i*epsilon) applied to the dolfinx LinearProblem restores '
                            '~10 iterations per convergence. (Audit 2026-06-02.)',
 ...
 'maxwell': {'description': "Maxwell's equations for electromagnetic wave propagation. "
             ...
             'pitfalls': ['[Physics] MUST use H(curl) elements (Nedelec / N1curl) for '
                          ...
                          "kind H(curl)') have DOF ordering by edge, not by node. "
                          'Setting tangential BCs requires interpolating onto the edge '
                          'basis, not the nodal basis. Signal: dirichletbc on an HCurl '
                          'space defined with a vector-valued function silently sets '
                          'only the first component on each edge, leaving the '
                          'tangential trace 90 degrees off from intended; '
                          'post-processed E field has non-zero normal component on the '
                          'conductor boundary.']},
 ...
 'reaction_diffusion': {'description': 'Systems of coupled reaction-diffusion '
                        ...
                        'pitfalls': ['[API] Build the mixed space with '
                                     ...
                                     'after 200 iterations. IMPORTANT CORRECTION to '
                                     'the previously written claim: the Picard '
                                     'residual ratio is NOT a steady ~0.5 per '
                                     'iteration -- it alternates, roughly 0.24 then '
                                     '2.78 then 0.28 then 2.69, and the envelope '
                                     'shrinks by only about 0.92 per iteration '
                                     'asymptotically. Same behaviour on triangles and '
                                     'quadrilaterals at P1 and P2.',
                                     ...
                                     'to 3, 1, 0, 0, 0, 0, 0, 0, 0, 0 -- the solver is '
                                     're-converging on a problem it has already '
                                     'solved. Nothing is raised and the SNES reports '
                                     'converged reason 2 or 3 throughout, so the '
                                     'iteration count dropping to 0 is the only '
                                     'visible tell. Same on triangles and '
                                     'quadrilaterals at P1 and P2.']},
 'nearly_incompressible_elasticity': {'description': 'Mixed methods for nearly '
                                      ...
                                      'pitfalls': ['[Numerical] Low-order displacement '
                                                   ...
                                                   'does NOT. Signal: convergence-rate '
                                                   'test with P1/P0 dolfinx mixed '
                                                   'FunctionSpace stagnates at '
                                                   'first-order in displacement while '
                                                   'P2/P1 achieves second-order; '
                                                   'cross-check via the Mandel '
                                                   'benchmark — P2/P1 recovers the '
                                                   'analytic result to within 0.5%, '
 ...
 'fracture': {'description': 'Phase-field (variational) fracture in FEniCSx. A crack '
              ...
              'pitfalls': ['[Numerical] Irreversibility is not optional and its '
                           ...
                           'iteration cap generously - a cap of a few tens will '
                           'silently truncate exactly the step that matters and leave '
                           'a half-propagated crack. Verified on 2D triangles, 2D '
 ...
 'stokes_darcy': {'description': 'Coupled free-fluid / porous-medium flow: Stokes in '
                  ...
                  'pitfalls': ['[API] There is no built-in Stokes-Darcy, Darcy or '
                               ...
                               'length approaches sqrt(K/mu_eff) from above as K '
                               'decreases. Signal: numerical, not textual - with cells '
