...
_FENICS_KNOWLEDGE = {
    ...
    "poisson": {
        ...
        "pitfalls": [
            ...
            "[Physics] Pure-Neumann Poisson admits the constant null space — the solution is determined only up to a constant. Either pin one DOF (DirichletBC on a single point) or add a Lagrange multiplier enforcing mean(u) = 0. Signal: PETScKrylovSolver reports KSP convergence to a residual far above tolerance (the RHS has a non-zero component along the null space) or returns a solution with arbitrary additive constant from run to run.",
